...
import DoubleCheckMark from '../../images/double-check.svg';

export default function Message(props:any) : React.ReactElement {
	let color:any = '#a5b9c5';
	...
	checkMark = (
		<svg 
			viewBox="0 0 16 11" height="11" width="16"  color={ color }>
	...
	return (
		<MessageWrap direction={props.direction === 'left' ? 'start' : 'end'} id = { props.id }>
			<MessageStyled onClick={ () => {
				if (props.callback)
					props.callback();
			}}>
				<MessageText>{ props.children }</MessageText>
				<MessageInfo>
					<MessageHourSent> { props.time } </MessageHourSent>
					<MessageStatus> { checkMark }	</MessageStatus>
				</MessageInfo>
			</MessageStyled>
		</MessageWrap>
	);
...
const MessageWrap = styled.div<{ direction : string }>`
	padding: 4px 5px;
	padding-bottom: 0;
	display: flex;
	justify-content: ${p => p.direction};
...
const MessageHourSent = styled.span``;
